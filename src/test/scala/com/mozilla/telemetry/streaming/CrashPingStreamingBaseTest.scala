...
class CrashPingStreamingBaseTest extends FlatSpec with Matchers with BeforeAndAfterEach with DataFrameSuiteBase {
  ...
  private val CrashPingStreaming = new CrashPingStreamingBase {
    ...
    override def buildOutputString(measurementName: String, timestamp: Long, buildId: String, tags: Map[String, String]): String = {
      List(measurementName, timestamp, buildId, tags).mkString(" ")
    }
  }
  ...
  "Parse ping" should "create a crash to send via HTTP" in {
    ...
    crashes
      .flatMap(m => CrashPingStreaming.parsePing(m, CrashPingStreaming.defaultChannels,
        CrashPingStreaming.defaultAppNames, defaultMeasurementName,
  ...
  "Message parser" should "generate 1 crash to send for each valid crash ping" in {
    ...
    val parsedPings = CrashPingStreaming.getParsedPings(spark.sqlContext.createDataset(messages).toDF,
  ...
  "Crash ping streaming" should "stream crash pings via HTTP" taggedAs Kafka.DockerComposeTag in {
