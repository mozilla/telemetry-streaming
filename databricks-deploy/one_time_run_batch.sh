...
CHECKPOINT_PATH=NOT_USED


create_job_json() {
cat << EOF
{
    "run_name": "Error aggregates - v2 preview - backfill",
    "new_cluster": {
    ...
    "spark_jar_task": {
        ...
        "parameters": ["--outputPath","${OUTPUT_PATH}", "--from","20180509", "--to","20180509that", "--minPartitions","2500"]
    }
}
EOF
}
